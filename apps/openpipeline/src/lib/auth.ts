  ...
  providers.push(
    Credentials({
      ...
      async authorize(credentials) {
        ...
        const requestedRole = (credentials?.role as string)?.trim() || "editor";
        return {
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.permissions = getPermissionsForRole(OPENPIPELINE_PERMISSIONS, role);
      }
      return session;
    },
  },
