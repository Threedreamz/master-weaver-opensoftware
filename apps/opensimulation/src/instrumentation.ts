 ...
 *
 * MUST live at src/instrumentation.ts (not app-root) because opensimulation
...
export async function register() {
  ...
  try {
    if (!existsSync(dataDir)) {
      ...
      console.log(`[opensimulation:boot] created data dir ${dataDir}`);
    }
    ...
    // migrations at `/app/apps/opensimulation/drizzle/migrations`; read the
    // explicit OPENSIMULATION_MIGRATIONS_DIR env var in prod, fall back to
    // cwd-relative in dev.
    const migrationsFolder = process.env.OPENSIMULATION_MIGRATIONS_DIR
      ? resolve(process.env.OPENSIMULATION_MIGRATIONS_DIR)
      : process.env.NODE_ENV === "production"
        ? "/app/apps/opensimulation/drizzle/migrations"
        : resolve(process.cwd(), "drizzle/migrations");
    console.log(`[opensimulation:boot] running migrations from ${migrationsFolder}`);

    try {
      migrate(db as never, { migrationsFolder });
      console.log(`[opensimulation:boot] migrations complete`);
    } catch (migErr) {
      ...
      console.warn(
        `[opensimulation:boot] migrate() bailed on existing schema — re-applying pending migrations idempotently`,
      );
      ...
      for (const entry of journal.entries) {
        ...
        console.log(`[opensimulation:boot] applied migration ${entry.tag} (idempotent path)`);
      }
      console.log(`[opensimulation:boot] all migrations applied via recovery path`);
    }
  } catch (err) {
    console.error(`[opensimulation:boot] FATAL — db init failed:`, err);
    throw err;
  }
}
