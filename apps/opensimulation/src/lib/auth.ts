...
// opensimulation reuses OPENCAD_PERMISSIONS for M1 — both apps share the same
// role model (viewer/editor/admin). If/when opensim needs distinct grants,
// add OPENSIMULATION_PERMISSIONS to @opensoftware/config/rbac and swap here.
const OPENSIMULATION_PERMISSIONS = OPENCAD_PERMISSIONS;
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.permissions = getPermissionsForRole(OPENSIMULATION_PERMISSIONS, role);
      }
      return session;
    },
  },
