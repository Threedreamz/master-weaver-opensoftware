 ...
 *
 * MUST live at src/instrumentation.ts (not app-root) because opendesktop
...
export async function register() {
  ...
  // opendesktop drizzle.config.ts uses `out: "./drizzle/migrations"` — SQL
  // files live at <app>/drizzle/migrations/<tag>.sql, _journal.json at
  // <app>/drizzle/migrations/meta/_journal.json. Dockerfile sets
  // WORKDIR=/app/apps/opendesktop at runtime so process.cwd() resolves
  // correctly in prod. dev runs from the same dir.
  const migrationsFolder = path.join(process.cwd(), "drizzle", "migrations");
  console.log(`[opendesktop:boot] running migrations from ${migrationsFolder}`);

  try {
    try {
      migrate(db as never, { migrationsFolder });
      console.log(`[opendesktop:boot] migrations complete`);
    } catch (migErr) {
      ...
      console.warn(
        `[opendesktop:boot] migrate() bailed on existing schema — re-applying pending migrations idempotently`,
      );
      const { readFileSync } = await import("fs");
      const { join } = await import("path");
      // opendesktop's src/db/index.ts directly opens better-sqlite3 but does
      // not export the raw sqlite handle. We open a sibling read/write handle
      // here for the journal-walk recovery path. The handle is short-lived
      // (only the lifetime of register()) and uses the same DATABASE_URL env.
      ...
      try {
        ...
        for (const entry of journal.entries) {
          ...
          console.log(`[opendesktop:boot] applied migration ${entry.tag} (idempotent path)`);
        }
        console.log(`[opendesktop:boot] all migrations applied via recovery path`);
      } finally {
        try {
          sqlite.close();
        } catch {
    ...
    console.warn("[opendesktop:boot] Auto-migration failed:", (err as Error).message);
  }
}
