...
export async function checkQuota(
  ...
  if (cls.perDay === 0) {
    return {
      ...
      reason: `userClass "${userClass}" is not permitted to generate images`,
    };
  }
  ...
  if (userDay >= cls.perDay) {
    return {
      ...
      used: userDay,
      resetAt: periodResetMs("day", now),
      reason: `daily image-generation limit for userClass "${userClass}" reached on provider "${provider}"`,
    };
  }
  ...
  if (userMonth >= cls.perMonth) {
    return {
      ...
      reason: `monthly image-generation limit for userClass "${userClass}" reached on provider "${provider}"`,
    };
  }
