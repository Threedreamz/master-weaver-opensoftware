 ...
 *
 * MUST live at src/instrumentation.ts (not app-root) because openfarm uses
...
export async function register() {
  ...
  // openfarm drizzle.config.ts uses `out: "./drizzle/migrations"` — SQL files
  // live at <app>/drizzle/migrations/<tag>.sql, _journal.json at
  // <app>/drizzle/migrations/meta/_journal.json. Dockerfile sets
  // WORKDIR=/app/apps/openfarm at runtime so process.cwd() resolves correctly
  // in prod. dev runs from the same dir.
  const migrationsFolder = path.join(process.cwd(), "drizzle", "migrations");
  console.log(`[openfarm:boot] running migrations from ${migrationsFolder}`);

  try {
    try {
      migrate(db as never, { migrationsFolder });
      console.log(`[openfarm:boot] migrations complete`);
    } catch (migErr) {
      ...
      console.warn(
        `[openfarm:boot] migrate() bailed on existing schema — re-applying pending migrations idempotently`,
      );
      const { readFileSync } = await import("fs");
      const { join } = await import("path");
      // openfarm's src/db/index.ts directly opens better-sqlite3 but does not
      ...
      try {
        ...
        for (const entry of journal.entries) {
          ...
          console.log(`[openfarm:boot] applied migration ${entry.tag} (idempotent path)`);
        }
        console.log(`[openfarm:boot] all migrations applied via recovery path`);
      } finally {
        try {
          sqlite.close();
        } catch {
    ...
    console.warn("[openfarm:boot] Auto-migration failed:", (err as Error).message);
  }
}
