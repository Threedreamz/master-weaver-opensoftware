...
export default function Page() {
  ...
  return (
    <main style={{ fontFamily: "system-ui, sans-serif", padding: "2rem", maxWidth: 720 }}>
      <h1 style={{ marginTop: 0 }}>OpenGen Gateway</h1>
      <p>
        AI 3D generation API. POST to <code>/api/generate</code> to submit a job, then GET{" "}
        <code>/api/jobs/&lt;id&gt;</code> to poll status.
      </p>
      <h2>Enabled providers</h2>
      {enabled.length === 0 ? (
        <p>
          <em>No providers enabled — set provider API keys in environment.</em>
        </p>
      ) : (
        <ul>
          {enabled.map((p) => (
