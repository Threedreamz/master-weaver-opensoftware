...
// import route modules and transitively this file. Even with a 15s busy_timeout
// the `journal_mode = WAL` pragma (which takes an exclusive lock) races across
...
// "better-sqlite3 + Next.js 16 page-data collection races".
function openWithRetry(path: string, maxAttempts = 20): Database.Database {
  ...
  throw lastErr instanceof Error
    ? lastErr
    : new Error(`opengen-gateway db open failed after ${maxAttempts} retries`);
}
