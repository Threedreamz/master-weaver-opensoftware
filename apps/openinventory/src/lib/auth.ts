...
// OpenInventory uses the same RBAC structure as other apps.
// Define inventory-specific permissions inline until added to config package.
const OPENINVENTORY_PERMISSIONS: Record<Exclude<UserRole, "admin">, string[]> = {
  editor: [
    "inventory:articles:read",
    "inventory:articles:write",
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.permissions = getPermissionsForRole(OPENINVENTORY_PERMISSIONS, role);
      }
      return session;
    },
  },
