...
export async function register() {
  ...
  // opensem drizzle.config.ts uses `out: "./drizzle"` — SQL files live at
  // <app>/drizzle/<tag>.sql, _journal.json at <app>/drizzle/meta/_journal.json.
  // Dockerfile sets WORKDIR=/app/apps/opensem at runtime so process.cwd()
  // resolves correctly in prod. dev runs from the same dir.
  const migrationsFolder = path.join(process.cwd(), "drizzle");
  console.log(`[opensem:boot] running migrations from ${migrationsFolder}`);

  try {
    try {
      migrate(db as never, { migrationsFolder });
      console.log(`[opensem:boot] migrations complete`);
    } catch (migErr) {
      ...
      console.warn(
        `[opensem:boot] migrate() bailed on existing schema — re-applying pending migrations idempotently`,
      );
      ...
      for (const entry of journal.entries) {
        ...
        console.log(`[opensem:boot] applied migration ${entry.tag} (idempotent path)`);
      }
      console.log(`[opensem:boot] all migrations applied via recovery path`);
    }
  } catch (err) {
    ...
    console.warn("[opensem:boot] Auto-migration failed:", (err as Error).message);
  }
}
