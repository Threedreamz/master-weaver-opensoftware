 ...
 *
 * MUST live at src/instrumentation.ts (not app-root) because openaccounting
...
export async function register() {
  ...
  // openaccounting drizzle.config.ts uses `out: "./drizzle"` — SQL files live
  // at <app>/drizzle/<tag>.sql, _journal.json at <app>/drizzle/meta/_journal.json.
  // Dockerfile sets WORKDIR=/app/apps/openaccounting at runtime so process.cwd()
  // resolves correctly in prod. dev runs from the same dir.
  const migrationsFolder = path.join(process.cwd(), "drizzle");
  console.log(`[openaccounting:boot] running migrations from ${migrationsFolder}`);

  try {
    try {
      migrate(db as never, { migrationsFolder });
      console.log(`[openaccounting:boot] migrations complete`);
    } catch (migErr) {
      ...
      console.warn(
        `[openaccounting:boot] migrate() bailed on existing schema — re-applying pending migrations idempotently`,
      );
      ...
      for (const entry of journal.entries) {
        ...
        console.log(`[openaccounting:boot] applied migration ${entry.tag} (idempotent path)`);
      }
      console.log(`[openaccounting:boot] all migrations applied via recovery path`);
    }
  } catch (err) {
    ...
    console.warn("[openaccounting:boot] Auto-migration failed:", (err as Error).message);
  }
}
