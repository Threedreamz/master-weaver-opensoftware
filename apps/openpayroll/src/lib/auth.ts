...
import {
  ...
  OPENACCOUNTING_PERMISSIONS,
} from "@opensoftware/config/rbac";

// Use OPENPAYROLL_PERMISSIONS if available, fall back to OPENACCOUNTING_PERMISSIONS
const APP_PERMISSIONS = OPENACCOUNTING_PERMISSIONS;
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.permissions = getPermissionsForRole(APP_PERMISSIONS, role);
      }
      return session;
    },
  },
