...
-- NextAuth WebAuthn / passkeys — shipped for future use. Schema matches the
-- @auth/drizzle-adapter canonical table; if shared.schema.ts never declared it,
-- the runtime won't touch it.
...
-- ==================== OPENCAD M2: ASSEMBLIES ====================
CREATE TABLE `opencad_assemblies` (
	`id` text PRIMARY KEY NOT NULL,
	`project_id` text NOT NULL,
	`root_tree_json` text NOT NULL,
	`created_at` integer DEFAULT (unixepoch()) NOT NULL,
	`updated_at` integer DEFAULT (unixepoch()) NOT NULL,
	FOREIGN KEY (`project_id`) REFERENCES `opencad_projects`(`id`) ON UPDATE no action ON DELETE cascade
);
...
--> statement-breakpoint
CREATE INDEX `opencad_tools_user_idx` ON `opencad_tools` (`user_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `opencad_posts_user_idx` ON `opencad_gcode_posts` (`user_id`);
