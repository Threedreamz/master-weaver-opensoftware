...
export function verifySpendeskWebhook(
  payload: string | Buffer,
  signatureHeader: string,
  config: SpendeskWebhookConfig,
  ...
  const event: SpendeskWebhookEvent = JSON.parse(rawPayload);
...
export type SpendeskWebhookHandler = (event: WebhookEvent) => Promise<void>;

export type SpendeskWebhookHandlerMap = Partial<
  Record<SpendeskWebhookEventType, SpendeskWebhookHandler>
>;

export async function handleSpendeskWebhook(
  payload: string | Buffer,
  signatureHeader: string,
  config: SpendeskWebhookConfig,
  handlers: SpendeskWebhookHandlerMap,
): Promise<{ handled: boolean; eventType?: string; error?: string }> {
  const result = verifySpendeskWebhook(payload, signatureHeader, config);

  if (!result.valid || !result.event) {
    return { handled: false, error: result.error };
  }

  const handler = handlers[result.event.type as SpendeskWebhookEventType];
  if (!handler) {
