...
export function detectOverhangs(
  ...
  for (let i = 0; i < triangles.length; i++) {
    ...
    let overhangAngle = 0;
    if (dotUp < 0) {
      // Face points downward — compute overhang angle from horizontal
      // acos(dotUp) gives angle from Z-up (>90° for downward faces)
      // Subtract 90° to get the overhang angle from horizontal:
      // straight down = 90°, 45° below horizontal = 45°, horizontal = 0°
      overhangAngle = Math.acos(clamp(dotUp, -1, 1)) * RAD2DEG - 90;
    }
